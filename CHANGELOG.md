...
## 2026-03-14 - 边界探测修复与增量提交加固

### ✅ 核心修复

1. **修复边界探测被误跳过**
   - 场景：`last_probed_id` 位于 `missing_ids`（如 758），实际文件最大 ID 为前一位（757）
   - 旧行为：被判定为“文件丢失”，直接跳过探测，后续新文章无法继续发现
   - 新行为：识别为“已知缺失边界”，继续从 `last_probed_id + 1` 增量探测
   - 结果：已成功继续探测到新边界（示例运行：758 → 843）

2. **修复工作流统计在 0 匹配时的失败风险**
   - 为文章变更统计命令增加 `status_output` 缓存与 `grep ... || true`
   - 避免 GitHub Actions 在 `-e -o pipefail` 下因无匹配直接报错退出

### ✅ 提交策略说明（保留你的目标）

- 仅在检测到“文章实体变更”（`docs/全部研报/*.md`，排除 `README.md`）时才提交
- 无文章新增/更新时，跳过元数据更新与 commit
- 这样可避免“0 文章更新但每天都有 commit”的噪音记录

### 📚 文档同步

- 更新 `README.md`：明确“增量探测 + 增量下载，不会每次全量重爬”
- 更新 `ARCHITECTURE.md`：补充工作流条件提交逻辑与本次修复说明
