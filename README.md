...
日常 Action 采用“列表 V1 优先、V2 Pro 受控兜底”的双层发现策略：上午只尝试普通额度 V1，下午允许在 V1 失效时回退 V2，每周再用 V2 做一次低频安全核查。这样 V1 恢复后可自动减少 Pro 消耗，V1 持续失效时也把 V2 控制在约 60 次/月；一次性历史缺口仍使用 V2 持久化游标补齐。RapidAPI 返回的长链接会与旧版短链接按标题和发布日期交叉去重，不会重复生成文章。
