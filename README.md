...
自动化每天上午、下午分别通过 RapidAPI 检查两个公众号，并为每个来源维护独立索引。多个 API Key 按日期轮换并自动故障转移，不再依赖微信读书扫码登录。新文章通过详情接口取得正文 HTML，封面与正文图片全部本地化后才会进入生产站；下载失败的内容会保留到下一次重试，纯图片文章也必须确认图片完整后才算归档完成。
