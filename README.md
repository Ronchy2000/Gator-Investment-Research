...
**鳄鱼派投资档案**是一个面向长期阅读的静态资料库。它把同一创作者运营的两个公众号放在一处：“获得信息差”负责每日信息整理，“像鳄鱼一样思考”负责每日市场复盘；同时保留旧站积累的 **913 篇公开研报**。截至 `2026-08-05` 已归档 **639 篇公众号文章**，正文和图片随站点一同保存，不必在聊天记录、收藏夹和失效链接之间反复寻找。
...
| 自 `2026-06-15` 起已归档 51 篇，支持图文与纯图片文章。 | 截至 `2026-08-05` 已归档 588 篇，最早至 `2023-07-22`，历史补录已到达接口末页。 | 支持分类、年份筛选和全文检索，历史内容冻结保存。 |
...
自动化每天北京时间 `10:17`、`18:47` 分别通过 RapidAPI 检查两个公众号，并为每个来源维护独立索引。API Key 池按日期轮换；遇到无权限、额度耗尽或限流时自动切换，不再依赖微信读书扫码登录。新文章通过详情接口取得正文 HTML，封面与正文图片全部本地化后才会进入生产站；下载失败的内容会保留到下一次重试，纯图片文章也必须确认图片完整后才算归档完成。

日常 Action 只读取 V1 最新列表页，避免无意义消耗第三方额度；一次性历史缺口已通过 V2 游标接口补齐。此后两个公众号的新文章全部由 GitHub Actions 自动检查和归档。RapidAPI 返回的长链接会与旧版短链接按标题和发布日期交叉去重，不会重复生成文章。
