...
| 自 `2026-06-15` 起持续同步，支持图文与纯图片文章。 | 已归档接口当前可见的最近 99 篇，并从此持续增量。 | 支持分类、年份筛选和全文检索，历史内容冻结保存。 |
...
自动化每天分别检查两个公众号，并为每个来源维护独立索引。新文章成功入库后才会进入生产站；下载失败的内容会保留到下一次重试，纯图片文章也必须确认图片完整后才算归档完成。

“像鳄鱼一样思考”显示共有 562 篇文章，但当前微信读书中转接口仅返回最近 99 篇，继续翻页会得到空列表。因此仓库已完整保存接口当前可见范围，并从此持续接住每日新增；无法枚举的更早文章不会被宣称为已归档。
