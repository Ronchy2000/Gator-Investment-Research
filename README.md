...
自动化每天通过 RapidAPI 检查两个公众号，并为每个来源维护独立索引。GitHub cron 明确使用 UTC：配置为 `23:30 UTC` 和 `07:57 UTC`（北京时间次日 `07:30` 和当日 `15:57`），结合近期调度延迟，目标实际启动窗口约为北京时间 `09:00` 和 `16:30`。三套产品与 API Key 分别轮换；某个 Key 未订阅某套产品时只跳过该组合，遇到产品上游故障则切换另一套，不再依赖微信读书扫码登录。新文章优先从微信原文保留完整 DOM 顺序，原文受限时只回退到已验证能保持图文顺序的详情接口；封面与正文图片全部本地化后才会进入生产站。下载失败的内容会保留到下一次重试，纯图片文章也必须确认图片完整后才算归档完成。
