...
自动化每天通过 RapidAPI 检查两个公众号，并为每个来源维护独立索引。GitHub cron 明确使用 UTC：配置为 `23:30 UTC` 和 `03:17 UTC`（北京时间次日 `07:30` 和当日 `11:17`）。结合近期实测调度延迟，目标实际启动窗口约为北京时间 `09:00` 和 `16:15–17:00`。三套产品与 API Key 分别轮换；某个 Key 未订阅某套产品时只跳过该组合，遇到产品上游故障则切换另一套，不再依赖微信读书扫码登录。新文章优先从微信原文保留完整 DOM 顺序，原文受限时只回退到已验证能保持图文顺序的详情接口；封面与正文图片全部本地化后才会进入生产站。下载失败的内容会保留到下一次重试，纯图片文章也必须确认图片完整后才算归档完成。

日常 Action 轮询三套产品的列表接口，让彼此独立的套餐额度共同参与文章发现；正文优先使用微信原文，回退时使用 Official Accounts Platform 或 WeChat Data。SIAN 的详情响应会重排原文图片，因此仅用于列表发现。已弃用的历史文章 V1 不再探测；一次性历史缺口仍固定使用 Official Accounts Platform V2 的持久化游标补齐，避免混用三种分页格式。RapidAPI 返回的长链接会与旧版短链接按标题和发布日期交叉去重，不会重复生成文章。
...
公众号下载能力参考并受益于 [x554960766/wechat-mp-tools](https://github.com/x554960766/wechat-mp-tools)，在此向作者 **xuyi** 表示感谢。当前生产同步轮询 RapidAPI 上三套微信公众号数据产品获取公开文章列表，正文则优先使用微信原文并由两套可信详情产品兜底；单个 Key 未订阅其中某套产品时会只跳过该组合，不影响其余产品。
