...
日常 Action 通过 V2 空游标读取最新列表页，一次性历史缺口也已通过 V2 持久化游标补齐。两个公众号的新文章全部由 GitHub Actions 自动检查和归档；RapidAPI 返回的长链接会与旧版短链接按标题和发布日期交叉去重，不会重复生成文章。已弃用的列表 V1 自 `2026-08-08` 起返回 `API INVALID`，生产流程已停止调用。
