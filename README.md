...
自动化每天通过 RapidAPI 检查两个公众号，并为每个来源维护独立索引。考虑 GitHub 定时调度器的实测延迟，计划任务配置为北京时间 `07:07`、`15:37`，目标实际启动窗口约为 `09:00` 和 `16:00` 左右。API Key 池按日期轮换；遇到无权限、额度耗尽或限流时自动切换，不再依赖微信读书扫码登录。新文章通过详情接口取得正文 HTML，封面与正文图片全部本地化后才会进入生产站；下载失败的内容会保留到下一次重试，纯图片文章也必须确认图片完整后才算归档完成。
