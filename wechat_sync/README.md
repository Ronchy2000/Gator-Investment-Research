...
GitHub Action 不启用 `--history-v2`。北京时间 `09:07` 和 `16:37` 的任务都从空游标读取 V2 最新页；显式 `--history-v2` 只用于本地持久化历史断点。两个公众号每天检查两次约消耗 120 次 V2 Pro 调用/月，按每个 Key 10 次计算至少需要 12 个有效 Key，建议准备 15 个以覆盖 31 天、手动执行和失败重试。
