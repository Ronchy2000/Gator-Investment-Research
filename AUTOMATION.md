...
实测第一个账号仅取得“像鳄鱼一样思考”最近 99 篇；后续通过有序账号池分批补档至 448 篇，最早覆盖到 `2024-08-23`。目前账号池包含 4 个账号；逐账号核查第 1–12 页后，可见文章的合并集合仍与本地 448 篇完全一致。中转服务对不同账号开放的历史窗口会延迟、波动且不连续，并非固定的“每账号 100 篇”。

公众号显示总数为 562。中转服务对不同账号开放的历史页可能不连续，同一账号可能出现“空页之后再次出现文章”的稀疏窗口。只要本地完成数仍低于这个已知总数，后续 Action 就会在预计的 12 页范围内循环探测；新增账号会从第 1 页重新扫描，单个空页不会结束历史回补。系统仍只把实际下载成功的文章计入归档。
