...
- 数据源：RapidAPI 的文章列表 V2 与文章详情 V4；已停用失效的列表 V1。
- 公众号：“获得信息差”和“像鳄鱼一样思考”。
...
| `/api/weixin/get-account-history-articles/v1` | `POST`，查询参数 `url`、`page` | 已弃用的旧文章列表 | `2026-08-08` 起返回 `HTTP 503 / code 300 API INVALID`，生产代码不再调用 |
| `/api/weixin/get-account-history-articles/v2` | `POST`，表单字段 `url`、`offset` | 使用 `PagingInfo.Offset` 读取最新页或连续历史 | Action 从空游标读取最新页；本地历史补录从保存的游标续传 |
...
截至 `2026-08-08`，实测免费套餐每个 Key 有 50 次普通月额度，V2 另有 10 次 Pro 月额度；套餐和额度可能调整，以 RapidAPI 页面及响应头为准。当前工作流通常每月需要：
...
列表请求同时受 Pro 子额度限制，因此每天两次检查两个公众号需要约 120 次 Pro 调用/月，理论上需要 12 个每月 10 次 Pro 额度的 Key；详情 V4 还会消耗普通额度，实际数量随发文和重试次数变化。不要用多个 Key 并行请求；同步器会按日期分摊使用，并仅在当前 Key 不可用时按顺序故障转移。更稳妥的方案是使用允许足够调用量的付费套餐，或降低定时频率。
...
- 业务码 `301`：第三方采集临时失败。
- 业务码 `300` 或 `HTTP 503`：接口本身无效或上游服务故障；新增 Key 通常无效。

只有所有 Key 都失败时，任务才失败并创建或更新“微信公众号自动同步失败”Issue。HTTP `429` 是额度用完后切换 Key 的主要信号；不需要人工调整 Key 顺序。

## 6. 历史补录

日常定时任务通过 V2 从空游标检查最新 `1` 页；每个公众号每次至少消耗 1 次 Pro 调用。同步器不会把这次增量游标写入历史断点，下一轮仍从最新页开始。只有两次任务之间新增超过一页时，才需要在手动运行界面临时提高 `max_pages`。
...
截至 `2026-08-08`，免费套餐响应头显示每个 Key 有 50 次普通月额度，但 V2 另受每月 10 次的 Pro 子额度限制。GitHub Action 和历史补录都会使用 V2；历史补录应按每个 Key 最多约 8 页分批运行，保留额度给日常增量和重试，并查看 RapidAPI 控制台用量。
...
| HTTP 503 / 业务码 300 `API INVALID` | 上游接口失效或服务故障 | 先核对端点版本；所有 Key 一致时报错时不要继续注册 Key |
