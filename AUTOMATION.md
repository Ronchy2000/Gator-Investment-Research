...
- 数据源：RapidAPI 的文章列表 V1/V2 与文章详情 V4；V1 优先，V2 Pro 受控兜底。
- 公众号：“获得信息差”和“像鳄鱼一样思考”。
...
RAPIDAPI_KEYS
  -> 按日期选择一个 Key
  -> 优先通过 V1 查询两个公众号的最新文章页
  -> V1 失效时按本轮额度策略回退 V2 Pro
  -> Key 无效、限流或额度耗尽时自动切换
...
| `/api/weixin/get-account-history-articles/v1` | `POST`，查询参数 `url`、`page` | 普通额度的较新文章列表 | 每次增量检查优先尝试；当前失效时自动降级，恢复后自动重新启用 |
| `/api/weixin/get-account-history-articles/v2` | `POST`，表单字段 `url`、`offset` | 使用 `PagingInfo.Offset` 读取最新页或连续历史 | 下午允许作为 Pro 兜底；每周核查 V1；本地历史补录从保存的游标续传 |
| `/api/weixin/get-article-detail/v4` | `GET`，查询参数 `articleUrl` | 返回标题、公众号、摘要和完整正文 HTML | 每篇新文章都通过它取得可归档正文 |

V1 和 V2 在这里指的是两代“公众号历史文章列表”接口，不是正文详情版本。Convert V1 只做链接转换；它不能代替文章列表 V2，也不能代替详情 V4。完整请求示例、返回字段和历史补录操作见 [wechat_sync/README.md](wechat_sync/README.md)。
...
截至 `2026-08-08`，实测免费套餐每个 Key 有 50 次普通月额度，V2 另有 10 次 Pro 月额度；套餐和额度可能调整，以 Action 日志中的响应头及 RapidAPI 页面为准。当前双层调度为：

```text
09:07：两个公众号只尝试 V1，不允许 V2 兜底
16:37：两个公众号先尝试 V1，失败时允许 V2 兜底
每周日下午：V1 成功时也用 V2 做一次安全核查
2 个公众号 × 每天约 1 篇新文章 × 30 天 = 约 60 次详情请求
```

V1 持续失效时，V2 兜底上限约为 `2 个公众号 × 1 次/日 × 30 天 = 60` 次 Pro 调用；每周安全核查只在 V1 成功时执行，因此不会和“V1 持续失效”的上限叠加。现有 10 个 Key 按每 Key 10 次 Pro 计算有约 100 次容量，留有失败重试余量。详情 V4 消耗普通额度，实际数量随发文和重试次数变化。不要用多个 Key 并行请求；同步器会按日期分摊，并仅在当前 Key 明确无权限或额度耗尽时按顺序故障转移。
...
业务码 `300/301`、HTTP `5xx`、网络超时或连接失败属于端点/上游故障，不再对全部 Key 重复同一请求。V1 出现这些故障时按运行策略进入 V2；V2/V4 出现故障时等待下次任务。HTTP `429` 是额度用完后切换 Key 的主要信号。

## 6. 历史补录

日常定时任务先通过 V1 检查最新 `1` 页。上午任务使用 `--no-v2-fallback`，V1 失效时只记录跳过，不创建每日噪音 Issue；下午任务允许 V2 从空游标兜底，保证每天至少一次可靠检查。星期日下午还使用 `--v2-audit` 比对 V1/V2，防止 V1 返回 HTTP 200 但缓存陈旧。增量游标不会写入历史断点。
...
截至 `2026-08-08`，免费套餐响应头显示每个 Key 有 50 次普通月额度，但 V2 另受每月 10 次的 Pro 子额度限制。GitHub Action 只在下午兜底或每周核查时使用 V2；历史补录应按每个 Key 最多约 8 页分批运行，保留额度给日常增量和重试，并查看 Action 日志中的额度头。
...
只有 `backfillComplete` 为 `true`、`backfillOffset` 为空字符串、`pendingCount` 为 `0` 时，才表示列表已到达接口末页且所有文章下载完整。不要用公众号页面显示的固定文章总数判断完成：清理 7 份旧编码损坏的重复归档后，本次 V2 末页对应 581 篇唯一文章，仍多于此前显示的 562 篇。截至 `2026-08-05`，“像鳄鱼一样思考”已满足以上三项条件，最早文章为 `2023-07-22`，不需要继续运行 V2；今后的新文章完全交给每天两次的 Action。
...
| V1 的 HTTP 503 / 业务码 300 `API INVALID` | V1 上游接口仍失效 | 上午正常跳过，下午自动使用 V2，无需新增 Key |
| 业务码 301 / HTTP 5xx / 网络超时 | 上游临时采集失败 | 不盲目轮询全部 Key；等待 V2 兜底或下次任务 |
...
日志会显示本轮使用 `V1 普通额度`、`V2 Pro 兜底`或`V2 Pro 安全核查`，并记录 RapidAPI 返回的额度头。若先显示“Key 池第 N/M 个不可用，尝试下一个”后同步成功，这是正常额度故障转移。只有全部 Key 都返回 `401/403/429` 时，才需要检查订阅、Key 或额度。
