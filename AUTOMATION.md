...
工作流文件为 `.github/workflows/wechat-sync.yml`，每天使用纯 UTC cron 各触发一次，也支持手动执行。GitHub 定时任务不保证准点：任务的 `createdAt` 与 `startedAt` 基本相同，延迟通常发生在任务创建前，而不是 runner 排队。2026-08-18 至 2026-08-26 的早间任务相对计划时间晚约 `87–95` 分钟，近期下午任务通常晚约 `20–35` 分钟。因此 cron 配置为 `23:30 UTC` 和 `07:57 UTC`，目标实际启动时间约为北京时间 `09:00` 和 `16:30`。不要增加 `timezone` 字段；调度时间统一按 UTC 计算，避免工作流重新注册后发生 8 小时时区偏移。
