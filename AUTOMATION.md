...
工作流文件为 `.github/workflows/wechat-sync.yml`，每天北京时间配置 `08:37` 和 `15:37` 各触发一次，也支持手动执行。GitHub 定时任务不保证准点：任务的 `createdAt` 与 `startedAt` 相同，延迟发生在任务创建前而不是 runner 排队。最近四次早间任务相对 `07:07` 晚 `23–47` 分钟，最近三次稳定在 `23–25` 分钟，因此早间 cron 调整到 `08:37`，目标实际启动时间约为 `09:00–09:25`；下午目标仍为 `16:00` 左右。
