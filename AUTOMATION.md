...
- 调度：北京时间每天配置 `07:07`、`15:37`，用于抵消 GitHub 调度延迟，目标实际启动窗口约为 `09:00`、`16:00`；也可手动运行。
...
工作流文件为 `.github/workflows/wechat-sync.yml`，每天北京时间配置 `07:07` 和 `15:37` 各触发一次，也支持手动执行。GitHub 定时任务不保证准点：近期任务的 `createdAt` 与 `startedAt` 相同，延迟发生在任务创建前而不是 runner 排队；上午实测晚约 2 小时，下午晚约 42–82 分钟，因此当前 cron 采用提前量，目标实际启动时间约为 `09:00–09:15` 和 `16:20–17:00`。
...
```text
07:07：计划触发，目标实际在 09:00 左右检查两个公众号
