...
`.github/workflows/wechat-sync.yml` 每天按 `23:30 UTC` 和 `03:17 UTC` 触发，也支持手动运行；对应北京时间次日 07:30 和当日 11:17。该提前量用于抵消 GitHub 调度器的实测延迟，目标实际启动窗口约为北京时间 09:00 和 16:15–17:00；两次任务均轮询三套产品检查两个公众号的最新文章，分别覆盖早间信息和收盘后复盘：
