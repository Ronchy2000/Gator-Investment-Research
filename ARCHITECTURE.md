...
7. 请求从账号池首开始，401、429 或历史空页时切换下一账号；新增账号会触发历史断点重探。

“像鳄鱼一样思考”首个登录账号可见最近 99 篇，第二个账号另行暴露 100 篇历史，当前共归档 199 篇。账号页面显示总数为 562；其余历史是否可枚举取决于后续账号与中转接口，完成索引只记录实际下载成功的内容。

第一页因中转缓存暂时为空时会最多重试三次。只有整个账号池均无法处理请求时，HTTP 401 或 429 才会使该公众号同步失败。
